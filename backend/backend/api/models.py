...
class TypicalChild(models.Model):
    def cdoc_path(instance, filename):
        return f'typical/t_{instance.sequence_no}_{instance.unique_no}/documents/{instance.cdoc_name}'

    def dgform_path(instance, filename):
        return f'typical/t_{instance.sequence_no}_{instance.unique_no}/documents/{instance.dgform_name}'
...
class AntypicalChild(models.Model):
    def cdoc_path(instance, filename):
        return f'antypical/at_{instance.clinic_no}/documents/{instance.cdoc_name}'

    def dgform_path(instance, filename):
        return f'antypical/at_{instance.clinic_no}/documents/{instance.dgform_name}'
...
class Audios(models.Model):
    def audio_upload_path(instance, filename):
        if instance.tChild:
            child = TypicalChild.objects.get(id=instance.tChild.id)
            session = Sessions.objects.get(id=instance.session.id)
            return f'typical/t_{child.sequence_no}_{child.unique_no}/sessions/{session.id}_{session.date}/audio/{instance.name}.{instance.file_extension}'

        else:
            child = AntypicalChild.objects.get(id=instance.atChild.id)
            session = Sessions.objects.get(id=instance.session.id)
            return f'antypical/at_{child.clinic_no}/sessions/{session.id}_{session.date}/audio/{instance.name}.{instance.file_extension}'
